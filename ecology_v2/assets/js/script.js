...
function calculateCompliance() {
    ...
    if (!volume || !zipCode || (volume == 'IDK' && !selfCart) || (volume == 'IDK' && selfCart == 'IDK' && !followUpAnswer)) {
        alert('Please answer the questions before submitting. Thank you!');
    } else {
        if (compliantZipCodes.includes(zipCode)) {
            window.scrollTo(0, document.body.scrollHeight);
            resultDiv.style.display = 'flex';
            resultDiv.innerHTML = complianceDate? `<h2>Your Compliance Date: ${complianceDate}</h2><p>${resources}</p>`: '<h2>Based on your answers, you are not required to comply with this regulation.</h2>';
        } else {
            resultDiv.style.display = 'flex';
